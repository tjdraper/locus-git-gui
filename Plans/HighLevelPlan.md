...
7. **Commit history and commit detail**
   ...
   Built and seen running in dark mode, driven by script: the rows, graph, labels, detail header, signatures, Find, Tab, the context menu, the palette's Go to Parent, commit windows and resizing. Not yet looked at by hand, or in light mode.
   ...
   - A commit graph beside the rows. Lane layout is a pure function of parent links, so it goes in the test target.
     ...
     - Every row's graph takes the width of the widest one read so far, so the subjects line up in one column, up to 12 lanes; past that the graph is cut off at the right rather than push the subjects out of the row
   ...
   - Performance check (see Decisions), with the test repositories this slice builds for every later slice to reuse:
     ...
     - With a commit-graph file, paging with `--skip` is fast enough even half a million commits down, so the long-running `git log` isn't needed. Without one, every page, search and refresh that reads the history again waits over four seconds on a repository this size. Still to decide: whether the app writes the file itself when it's missing, offers to, or leaves it to Git's own maintenance. It's a cache Git keeps for itself and changes nothing the user sees, but writing it takes a lock that a `git gc` running at the same moment would wait on.
     - Memory: 135 MB for 100,000 commits with their graph rows, about 1.4 KB each. Ordinary scrolling never reads that many, but reading to the end of a million commits, which Go to Parent from a very old merge can do, would hold about 1.4 GB. Still to decide: whether to hold less per commit, stop Go to Parent past some depth, or leave it.
