...
2. **Running Git and opening a repository**
   ...
   - How Git's failures are shown, built here so every later slice uses it (see Decisions):
     ...
     - A command log for each repository (View > Show Git Log, which slice 7 turns into View > Show Activity): every command the app ran, when, how long it took, its exit status and output. It's how someone checks what the app did, and what they paste into a bug report.
...
7. **Commit history and commit detail**
   ...
   Done. Seen running in light and dark mode, and tested against a generated history of a million commits and a blobless clone of the Linux kernel (see the performance check).
   ...
   - The middle column shows the history of whatever is selected in the sidebar: a branch, a remote branch, a tag, a stash. With nothing selected it shows the checked-out branch, or HEAD when detached. Selecting a remote shows the history of all its branches.
     ...
     - Reading a history again, such as after a commit, keeps the old rows until the new ones arrive, keeps the row at the top where it was, and keeps the selected commit selected when it's still there. A different history, or a search started or cleared, starts at the selected commit or the top instead.
   ...
   - Each row shows the subject, author, relative date and short hash, with branch and tag labels on the commits they point at
     - Built as a two-line row of the same height for every commit: the subject alone, then labels · author · date · short hash. The second line is cut from the right, so the least useful part goes first: the hash, then the date, then the author, and the labels last (`HistoryRowDetails`, in the test target). The hash and date are dropped whole rather than cut, since a partial hash looks like a real one. The author can end in an ellipsis. Labels that don't fit collapse into a +N label listing the rest in its tooltip.
     - HEAD is labelled only when detached; the checked-out branch's label is bold. Each label has its sidebar icon in front of its name: the branch icon, the checkmark for the checked-out branch, the tag icon, and a pin (`mappin`) for a detached HEAD, which has no sidebar row. Remote branches keep the sidebar's branch icon, and their `origin/` name and colour tell them from local ones. The detail column's labels have the same icons.
   - A commit graph beside the rows. Lane layout is a pure function of parent links, so it goes in the test target.
     ...
     - Every row's graph takes the width of the widest one read so far, so the subjects line up in one column
     - At most 8 lanes are drawn. The Linux kernel's history runs up to 165 lines side by side, which nobody can follow and which took the whole row. Past the eighth lane, everything is drawn in the last one: lines passing through become one grey line, a commit out there keeps its dot in its own colour, and lines into or out of that region bend to it. Only the row as drawn is kept, which also keeps a wide history's memory down (see the performance check). The graph's view is clipped to its column, since from macOS 14 a view draws outside its bounds unless told not to, which had put lines through the subjects.
   - Load history lazily as the list scrolls. A repository with a million commits opens as fast as one with ten.
     - Built: 200 commits first, then 1,000 at a time as the list nears the end, each page its own `git log --topo-order --skip`. That's only fast when the repository has a commit-graph file. Without one, `--topo-order` walks the whole history before printing anything, and each page walks it again. Git writes the file during `gc` but not on clone or fetch by default. The performance check below measured it with and without the file.
   ...
   - The detail column says whether the selected commit is signed and whether the signature checks out on this Mac, from `%G?` and `%GS`, read for the selected commit only and never in the history's rows, since each check runs GPG or `ssh-keygen`. It's read on its own, so the header and changes don't wait for it.
     ...
     - Built as `CommitSignature`, tested against real SSH-signed commits. Git reports an SSH signature it can't check, for want of an allowed signers file, as no signature at all (`N`) and says why only on standard error, so the commit itself is checked for a `gpgsig` header first. That also spares unsigned commits, most of them, a run of GPG. Every signed state's tooltip is what GPG or `ssh-keygen` said.
   ...
   - Performance check (see Decisions), with the test repositories this slice builds for every later slice to reuse:
     ...
     - Built: `Scripts/GenerateTestRepository.swift`, the kernel notes in `Scripts/README.md`, and `HistoryPerformanceTests`, which times the history's own reading and graph layout against a repository named by `LOCUS_PERFORMANCE_REPOSITORY` and is skipped without one. The kernel clone hasn't been made yet.
     - Measured on a generated repository of a million commits with eight lanes of merges (2026-09-25):

       | | With a commit-graph file | Without |
       |---|---|---|
       | First page of 200 | 23 ms | 4.6 s |
       | A page of 1,000 after 10,000 | 47 ms | 4.6 s |
       | A page of 1,000 after 100,000 | 85 ms | 4.6 s |
       | A page of 1,000 after 500,000 | 279 ms | 4.6 s |
       | 20,200 commits a page at a time | 0.96 s | 97 s |
       | Search by message or author, first page | 34–49 ms | 4.6 s |
       | Search in changes (`-S`), whole history | 5.3 s | 9.1 s |

       The app's own log agreed: 16 ms and about 45 ms a page with the file, 5 s and 4.6 s a page without. Writing the file took 8.8 seconds.
     - Measured on a blobless clone of the Linux kernel, 1,483,973 commits (2026-09-26):

       | | With a commit-graph file | Without |
       |---|---|---|
       | First page of 200 | 47 ms | 9.7 s |
       | A page of 1,000 after 10,000 | 89 ms | 9.4 s |
       | A page of 1,000 after 500,000 | 458 ms | 10 s |
       | 20,200 commits a page at a time | 2.0 s | 206 s |
       | Writing the file as the app does | 1 min 48 s | |

       A search that matches only a few commits reads the whole history either way, 11 to 17 seconds, since matching needs every commit's message, which the file doesn't hold. Search results now arrive as Git finds them, so the first of 17 rare matches showed after 2.4 seconds. A plain page still arrives whole, so a history read again finds its selected commit in it; a search keeps looking for the selected commit until it finishes.
     - With a commit-graph file, paging with `--skip` is fast enough even half a million commits down, so the long-running `git log` isn't needed. Without one, every page, search and refresh that reads the history again waits over four seconds on a repository this size. Decided: the app writes the file itself when a repository opens without one (see the item above).
     - Memory: 135 MB for 100,000 commits with their graph rows, about 1.4 KB each. On the kernel it was 564 MB, 5.9 KB each, since a row stored each of its average 109 lines; keeping only the row as drawn, at most 8 lanes, brought it to 134 MB. Ordinary scrolling never reads that many, but reading to the end of a million commits, which Go to Parent from a very old merge can do, would hold about 1.4 GB. Decided: Go to Parent reads at most 50,000 commits down, about 70 MB. Past that, or for a parent a search's results don't hold, or one that isn't in the history, it says so and offers the parent in a window of its own, which needs only that one commit read. Each commit stays the size it is.
     - Search in changes reads every commit's changes, so it takes seconds on a large history however it's run. The spinner covers it.
     - The weak spots listed before measuring: each page is its own `git log --skip`, so reading deep into a history walks the skipped commits again every page, and so does each page of a search. One long-running `git log` read a page at a time would avoid that, but needs a way to pause reading its output. Every commit read stays in memory, at roughly half a kilobyte each with its graph row, so scrolling to the end of the kernel holds hundreds of megabytes; rows could hold less and read the rest on demand. The numbers decide which of these is fixed here.
   - A repository that has no commit-graph file gets one written in the background as soon as it opens (`git commit-graph write --reachable --split --changed-paths`), rather than after something turns out slow. Without it, a large history is slow on every page and every search (see the performance check). It's a cache Git keeps for itself and changes nothing the user sees; `git gc` and `git maintenance` write the same file. `--changed-paths` makes a future file history and blame fast. It waits while another Git process is working in the repository, since writing it takes a lock.
     - Git doesn't add new commits to the file, but a file a little out of date is still fast. Slice 10 adds a layer after a fetch or pull, which bring in many commits at once.
     - Built as `CommitGraphWriter`, started after a repository window's first read. It waits up to a minute for Git processes the app didn't start, found through `GitProcessFinder`, which now leaves out the app's own. On the million-commit repository it took 20 seconds with `--changed-paths` (9 without), while the history's first page still took its 5; the pages after took 70 ms rather than 4.6 seconds, with no reopening. It's written once per window: a write that's cancelled or fails waits for the repository to be opened again, and a failure shows only in the Activity window, since the app works without the file.
   - Activity: a toolbar indicator on each repository window whenever the app is running a command there, and a window listing what's running and everything that ran
     - The Git log window becomes the Activity window, with running commands at the top, live and with how long they've been going, above the finished ones as before. What finished before the window opened is still there. View > Show Git Log becomes View > Show Activity.
     - The indicator shows the moment a command starts and stays half a second after the last one ends, so every command shows and a burst of quick ones reads as one rather than flickering. Clicking it opens the Activity window.
     - A running command has a Cancel button in the window. Slice 10's fetch, pull, push and clone cancel there too.
     - Built. Every command runs as a task of its own, so Cancel stops it whoever started it; Git gets the chance to clean up, and a cancelled commit-graph write left no lock or partial file behind. A cancelled command shows in grey rather than as a failure. Finished commands are listed newest finished first, so a long one that just ended is at the top, with the time it started.
     - AppKit only uses a toolbar's default items when it has no saved layout, so an item added in a later version never reaches a toolbar customized or saved before. `RepositoryToolbar` puts each new default item in once, before the warning, and remembers it offered it, so an item taken out in Customize Toolbar stays out. Slice 10's Fetch, Pull and Push arrive the same way.
   - A commit opens in a window of its own: double-click it in the history, press Return there, or choose Commit > Open in New Window. The window holds the same header and changes as the detail column, so it gets slice 8's diff view with no work of its own.
     ...
     - Clicking a parent in a commit window shows the parent in that window, which is what the user is reading, unless the parent already has a window of its own, which comes forward. Reveal in Sidebar brings the repository's window forward.
     - Built as `CommitWindowCoordinator`, one per repository window. A commit window's Commit menu has Copy Hash, Copy Subject, Go to Parent, Reveal in Sidebar and Show Full Message.
